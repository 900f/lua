...
export function buildLoaderScript(loaderKey, siteUrl, keyProtected, useKeySystem) {
  ...
  const tokUrl  = `${base}/api/key-system/token`;
...
local function _req(url, body)
  ...
  if not ok then
    print("[Luvenn] Request failed - pcall error")
    return nil
  end
  if not res then
    print("[Luvenn] No response object")
    return nil
  end
  if not res.Success then
    print("[Luvenn] Request not successful, status:", res.StatusCode)
    return nil
  end
  local ok2, data = pcall(function() return HS:JSONDecode(res.Body) end)
  if not ok2 then
    print("[Luvenn] Failed to parse JSON")
    return nil
  end
  return data
end`;
...
local td = _req("${tokUrl}", {key = "${loaderKey}", n = _n, h = _h})
local keyUrl = ""
