...
-- Fetch key page link
local tokenRes = pcall(function()
  local r = HS:RequestAsync({
    Url = "${base}/api/key-system/token",
    Method = "POST",
    Headers = {["Content-Type"]="application/json"},
    Body = HS:JSONEncode({key="${loaderKey}",n=_n,h=_h})
  })
  if r.Success then
    local d = HS:JSONDecode(r.Body)
    if d.url then
      sub.Text = "Complete the tasks at the link below to receive your key."
      linkBtn.MouseButton1Click:Connect(function()
        setclipboard(d.url)
        linkBtn.Text = "Link Copied!"
        task.delay(2, function() linkBtn.Text = "Copy Key Link" end)
      end)
      linkBtn.Text = "Copy Key Link"
    end
  end
end)
...
submitBtn.MouseButton1Click:Connect(function()
  ...
  local ok, err = pcall(function()
    ...
    local d = HS:JSONDecode(r.Body)
    if not d.s then error(d.e or "Invalid key") end
    sg:Destroy()
  ...
  if not ok then
    submitBtn.Text = tostring(err):sub(1,30)
    task.delay(3, function() submitBtn.Text = "Submit Key" end)
  end
end)`;
  }
