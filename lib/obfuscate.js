...
submitBtn.MouseButton1Click:Connect(function()
    ...
    local ok, err = pcall(function()
        ...
        if not r.Success then 
            error("HTTP " .. r.StatusCode .. ": " .. (r.StatusMessage or "Unknown error"))
        end
        ...
        if not fn then 
            error(le or "Failed to load script")
        end

        sg:Destroy()        -- Close the GUI before running
    ...
    if not ok then
        submitBtn.Text = tostring(err):sub(1, 35)
