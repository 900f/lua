...
local function _req(url, body)
  ...
  -- Method 1: http.request (Krnl, Script-Ware, etc.)
  ...
  -- Method 4: HttpGet (Some executors)
  if not response then
    local ok, res = pcall(function()
      if HS.HttpGet then
  ...
  -- Method 5: GetAsync (Roblox Studio style)
  if not response then
    local ok, res = pcall(function()
      if HS.GetAsync then
        return HS:GetAsync(fullUrl)
      end
      return nil
    end)
    if ok and res then
  ...
  -- Method 6: RequestAsync with GET
  if not response then
    local ok, res = pcall(function()
      if HS.RequestAsync then
...
`local function saveKey(key)
  -- Try different storage methods based on executor
  local saved = false
  
  -- Method 1: Synapse X data folder
  if syn and syn.io then
    local ok, err = pcall(function()
      local file = syn.io.open("luvenn_key.txt", "w")
      if file then
        file:write(key)
        file:close()
        saved = true
  ...
  -- Method 2: Krnl workspace (if writefile exists)
  if not saved and writefile then
    local ok, err = pcall(function()
      writefile("luvenn_key.txt", key)
      saved = true
    end)
  end
  
  -- Method 3: Script-Ware data
  if not saved and setclipboard then
    -- Fallback: just store in clipboard for next time (less ideal)
    pcall(function()
      setclipboard(key)
      print("[Luvenn] Key saved to clipboard. Paste it next time if needed.")
    end)
  end
  
  if saved then
    print("[Luvenn] Key saved successfully!")
  end
  return saved
end
...
local function loadSavedKey()
  ...
  if syn and syn.io then
    local ok, res = pcall(function()
      local file = syn.io.open("luvenn_key.txt", "r")
      if file then
    ...
    if ok and res and res ~= "" then key = res end
  end
  
  -- Method 2: Krnl
  if not key and readfile then
    local ok, res = pcall(function()
      return readfile("luvenn_key.txt")
    end)
    if ok and res and res ~= "" then key = res end
  end
  
  if key then
    print("[Luvenn] Loaded saved key:", key)
  end
  return key
end`;
...
-- KEY STORAGE FUNCTIONS (must be defined first)
local function saveKey(key)
  local saved = false
  if syn and syn.io then
    local ok, err = pcall(function()
  ...
  if not saved and setclipboard then
    pcall(function()
      setclipboard(key)
      print("[Luvenn] Key saved to clipboard.")
    end)
  end
  if saved then
    print("[Luvenn] Key saved successfully!")
...
-- Auto-fill saved key (NOW loadSavedKey exists)
local savedKey = loadSavedKey()
if savedKey then
  keyBox.Text = savedKey
  print("[Luvenn] Auto-filled saved key")
end
...
submitBtn.MouseButton1Click:Connect(function()
  ...
  local r = _req("${execUrl}", {k = k, n = _n, h = _h})
  
  if not r or not r.s then
    status.Text = (r and r.e) or "Invalid key."
    status.TextColor3 = Color3.fromRGB(200, 60, 60)
    submitBtn.Text = "Verify Key"
    submitBtn.BackgroundColor3 = Color3.fromRGB(24, 22, 38)
    return
  ...
  -- Save the valid key for next script run
  saveKey(k)
