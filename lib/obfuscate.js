...
submitBtn.MouseButton1Click:Connect(function()
  ...
  -- TEST: Simple GET request to a test endpoint
  local testUrl = "${base}/api/test"
  print("[Luvenn] Testing connection to:", testUrl)
  ...
  if testOk and testRes then
    print("[Luvenn] Test response status:", testRes.StatusCode)
    status.Text = "Connection OK. Verifying key..."
  else
    print("[Luvenn] Test failed")
    status.Text = "Cannot reach Luvenn servers. Check executor settings."
    status.TextColor3 = Color3.fromRGB(200, 60, 60)
    submitBtn.Text = "Verify Key"
    submitBtn.BackgroundColor3 = Color3.fromRGB(24, 22, 38)
    return
  ...
  -- Now try the actual key verification
  local getUrl = "${execUrl}?k=" .. k .. "&n=" .. _n .. "&h=" .. _h
  print("[Luvenn] Verifying key at:", getUrl)
  ...
  if not ok then
    status.Text = "Request failed: " .. tostring(ok)
    status.TextColor3 = Color3.fromRGB(200, 60, 60)
    submitBtn.Text = "Verify Key"
    submitBtn.BackgroundColor3 = Color3.fromRGB(24, 22, 38)
    return
  ...
  print("[Luvenn] Response status:", res.StatusCode)
  print("[Luvenn] Response body:", res.Body)
  
  if not res.Success then
    status.Text = "HTTP " .. tostring(res.StatusCode)
    status.TextColor3 = Color3.fromRGB(200, 60, 60)
    submitBtn.Text = "Verify Key"
    submitBtn.BackgroundColor3 = Color3.fromRGB(24, 22, 38)
    return
  ...
  local ok2, r = pcall(function() return HS:JSONDecode(res.Body) end)
