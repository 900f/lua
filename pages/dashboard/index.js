...
export default function Dashboard({ user }) {
  ...
  useEffect(() => {
    fetch('/api/executions/stats').then(r => r.json()).then(setStats);
    fetch('/api/executions/list?limit=6').then(r => r.json()).then(d => setRecents(d.executions || []));
  }, []);
    ...
    <Layout user={user}>
      ...
      <div className="stats-section">
        ...
        <div className="stats-grid">
          <Stat label="Total Scripts" value={stats?.scripts?.total} />
          <Stat label="Active" value={stats?.scripts?.active} accent />
          <Stat label="Key Protected" value={stats?.scripts?.key_protected} />
        </div>
      </div>
      ...
      <div className="stats-section">
        ...
        <div className="stats-grid">
          <Stat label="Total Checks" value={stats?.executions?.total} />
      ...
      {recents.length > 0 && (
        <div className="card">
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>Script</th><th>Roblox Name</th><th>IP</th><th>Status</th><th>Time</th></tr></thead>
              <tbody>
                {recents.map(e => (
                  <tr key={e.id}>
                    <td style={{ fontWeight: 500 }}>{e.script_name}</td>
                    <td>{e.roblox_name}</td>
                    <td className="td-mono">{e.ip_address}</td>
                    <td><span className={`badge ${e.success ? 'badge-green' : 'badge-red'}`}>{e.success ? 'Success' : 'Failed'}</span></td>
                    <td className="td-mono">{new Date(e.executed_at).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function Stat({ label, value, accent, sub }) {
  return (
    <div className="stat-card">
      ...
      <div className={`stat-value${accent ? ' accent' : ''}`}>{value ?? <span className="spinner" style={{ width: 16, height: 16 }} />}</div>
