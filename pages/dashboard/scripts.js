...
export default function Scripts({ user }) {
  ...
  async function del(s) {
    const ok = await confirm({
      title: `Delete "${s.name}"?`,
      message: 'This will permanently delete the script, all its keys, and execution logs. This cannot be undone.',
  ...
  async function toggle(s) {
    ...
    const r = await fetch(`/api/scripts/${s.id}`, {
      ...
      body: JSON.stringify({
        ...
        key_protected: sc.key_protected, use_key_system: sc.use_key_system,
        active: !s.active,
      }),
    });
  ...
  function getLoadstring(s) {
    ...
    if (s.use_key_system) return `loadstring(game:HttpGet("${url}",true))()`;
  ...
  return (
    <Layout user={user}>
                ...
                {scripts.map(s => (
                  <tr key={s.id}>
                    <td>
                      ...
                      {s.description && (
                        <div style={{ fontSize: 11.5, color: 'var(--text3)', marginTop: 2 }}>{s.description}</div>
                      )}
                    </td>
                    <td>
                      {s.use_key_system
                        ...
                        : s.key_protected
                          ? <span className="badge badge-accent">Key Lock</span>
      ...
      {showCreate && (
        <ScriptModal onClose={() => setShowCreate(false)} onDone={() => { setShowCreate(false); load(); toast('Script created!'); }} />
      )}
      {editScript && (
        <ScriptModal script={editScript} onClose={() => setEditScript(null)} onDone={() => { setEditScript(null); load(); toast('Script updated!'); }} />
      )}
      {lsScript && (
        <LoadstringModal script={lsScript} loadstring={getLoadstring(lsScript)} onClose={() => setLsScript(null)} siteUrl={siteUrl} />
      )}
    </Layout>
  );
}
...
function ScriptModal({ script, onClose, onDone }) {
  ...
  const [form, setForm] = useState({
    ...
    script_content: '', key_protected: script?.key_protected || false,
    use_key_system: script?.use_key_system || false, active: script?.active !== false,
  });
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg">
        ...
        <div className="modal-sub">{isEdit ? 'Update script content or settings' : 'Upload your Lua to generate a protected loadstring'}</div>
        {error && <div className="alert alert-error">{error}</div>}
        <form onSubmit={submit}>
          <div className="form-group">
            <label className="form-label">Script Name</label>
            <input className="form-input" placeholder="My Awesome Script" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} required />
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <div className="form-hint">Paste your full Lua script. It is stored encrypted server-side and never exposed in the loader file.</div>
            {loadingContent
              ? <div style={{ padding: 16, textAlign: 'center' }}><span className="spinner" /></div>
              : <textarea className="form-input form-textarea large" placeholder="-- Your Lua script here" value={form.script_content} onChange={e => setForm(f => ({ ...f, script_content: e.target.value }))} required />
            }
          </div>
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: 16, marginBottom: 16 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text2)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 14 }}>Protection Mode</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <label className="check-row">
                <input type="checkbox" checked={form.key_protected} onChange={e => setKP(e.target.checked)} />
                <div><div className="check-label">Key Protected</div><div className="check-hint">Users must set script_key = "..." before the loadstring. You generate keys manually.</div></div>
              </label>
              <label className="check-row">
                <input type="checkbox" checked={form.use_key_system} onChange={e => setKS(e.target.checked)} />
                <div><div className="check-label">Built-in Key System</div><div className="check-hint">An in-game menu appears — users complete tasks to receive a key automatically.</div></div>
              </label>
              {isEdit && (
                <label className="check-row">
...
function LoadstringModal({ script, loadstring, onClose, siteUrl }) {
  ...
  function copy() { navigator.clipboard.writeText(loadstring); setCopied(true); setTimeout(() => setCopied(false), 2200); }
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="modal-title">Loadstring — {script.name}</div>
        <div className="modal-sub">
          Share this with your users. The actual Lua source is never in this file.
          {script.key_protected && ' Replace YOUR_KEY_HERE with a real generated key.'}
        </div>
        ...
        <div className="alert alert-info" style={{ marginTop: 14, fontSize: 12 }}>
          ...
          <br /><strong>Security:</strong> URLs in the loader are XOR-encoded — the real endpoint is not readable.
        </div>
        <div className="modal-footer">
