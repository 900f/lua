...
export default function Executions({ user }) {
  ...
  async function load(sid) {
    ...
    try {
      ...
      toast('Failed to load executions: ' + e.message, 'error');
      setExecutions([]);
    }
    setLoading(false);
  }
  ...
  async function banIp(ip) {
    if (!confirm(`Ban IP ${ip}?\nThey will be blocked from all your scripts.`)) return;
    ...
    <Layout user={user}>
      ...
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">Every script run logged with IP, username and timestamp. Logs auto-delete after 3 days.</div>
        </div>
      </div>
      ...
      <div className="topbar">
        <select
          ...
          style={{ width: 'auto', fontSize: 13, padding: '7px 32px 7px 10px', height: 36 }}
          value={scriptFilter}
        ...
        <button className="btn btn-ghost btn-sm" onClick={() => load(scriptFilter)} title="Refresh">
          <RefreshIcon />
        </button>
      </div>
      ...
      <div className="card">
        {loading ? (
          <div style={{ padding: 56, textAlign: 'center' }}><span className="spinner" /></div>
        ) : filtered.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="empty-sub">
              {executions.length === 0
                ? 'Executions appear here when users run your scripts'
              ...
              <tbody>
                {filtered.map(e => (
                  <tr key={e.id}>
                    ...
                    <td>
                      {e.key_used
                        ? <span className="td-mono" style={{ fontSize: 10.5 }}>{e.key_used.slice(0, 14)}…</span>
