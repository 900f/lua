...
export default async function handler(req, res) {
  ...
  const rl = rateLimit('st:'+user.id, 20, 10000);
  if (!rl.allowed) return res.status(429).json({ error: 'Too many requests.' });
  const sql = getDb();
  if (req.method === 'GET') {
    const [s] = await sql`SELECT theme,dark_mode FROM user_settings WHERE user_id=${user.id} LIMIT 1`;
    return res.json({ theme: s?.theme||'violet', dark_mode: s?.dark_mode||false });
  }
  if (req.method === 'POST') {
    const theme = sanitize(req.body.theme||'violet', 20);
    const dark_mode = req.body.dark_mode === true;
    const valid = ['violet','pink','blue','green','orange'];
