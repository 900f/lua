...
export default async function handler(req, res) {
  ...
  const rl = rateLimit(`register:${ip}`, 3, 600000);
  if (!rl.allowed) {
    const waitSecs = Math.ceil((rl.resetAt - Date.now()) / 1000);
    return res.status(429).json({ error: `Too many registration attempts. Try again in ${waitSecs} seconds.` });
  }
  ...
  if (!/^[a-zA-Z0-9_]+$/.test(username)) return res.status(400).json({ error: 'Username: letters, numbers and underscores only.' });
