...
export default async function handler(req, res) {
  ...
  const ip = getIp(req);
  const rl = rateLimit('kss:'+ip, 30, 10000);
  ...
  return res.json({ roblox_name:kt.roblox_name, completed:kt.completed, key:kt.completed?kt.key_value:null, tasks:tasks.map(t=>({id:t.id,label:t.label,url:t.url,type:t.task_type,done:done.includes(t.id)})) });
}
