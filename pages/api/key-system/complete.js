...
export default async function handler(req, res) {
  ...
  const ip = getIp(req);
  const rl = rateLimit(`ks_complete:${ip}`, 20, 10000);
  ...
  const sql = getDb();
  const [kt] = await sql`
    SELECT id, script_id, tasks_completed, completed, expires_at
    FROM key_system_tokens WHERE token=${token} LIMIT 1
  `;
  ...
  if (allDone) {
    ...
    const { generateScriptKey } = await import('../../../lib/obfuscate');
    const key_value = generateScriptKey();
    await sql`
      INSERT INTO script_keys (script_id, user_id, key_value, note, duration)
      VALUES (${kt.script_id}, ${script.user_id}, ${key_value}, ${'Key System - auto'}, ${'lifetime'})
    `;
    await sql`
      UPDATE key_system_tokens SET tasks_completed=${JSON.stringify(done)}, completed=true, key_value=${key_value}
      WHERE id=${kt.id}
    `;
