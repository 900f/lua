...
export default async function handler(req, res) {
  ...
  const rl = rateLimit('kst:'+user.id, 20, 10000);
  ...
  if (req.method === 'POST') {
    ...
    const task_type = sanitize(req.body.task_type||'link', 30);
    const sort_order = parseInt(req.body.sort_order)||0;
    if (!label||!url) return res.status(400).json({ error: 'Label and URL required.' });
    const [task] = await sql`INSERT INTO key_system_tasks (script_id,user_id,task_type,label,url,sort_order) VALUES (${scriptId},${user.id},${task_type},${label},${url},${sort_order}) RETURNING id,task_type,label,url,sort_order,active`;
