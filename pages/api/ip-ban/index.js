...
export default async function handler(req, res) {
  ...
  const rl = rateLimit(`ipban:${user.id}`, 20, 10000);
  ...
  if (req.method === 'POST') {
    ...
    try {
      await sql`
        INSERT INTO ip_bans (user_id, ip_address, reason) VALUES (${user.id}, ${ip_address}, ${reason})
        ON CONFLICT (user_id, ip_address) DO UPDATE SET reason=${reason}
      `;
      return res.status(201).json({ ok: true });
    } catch (e) {
      return res.status(400).json({ error: 'Failed to ban IP.' });
    }
  }
