...
function getExpiry(duration) {
  ...
  const now = new Date();
  if (duration === 'day') return new Date(now.getTime() + 86400000);
  if (duration === 'week') return new Date(now.getTime() + 7 * 86400000);
  if (duration === 'month') return new Date(now.getTime() + 30 * 86400000);
  return null;
}
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const keys = await sql`
      SELECT id, key_value, hwid, hwid_locked, active, note, duration, expires_at, created_at, last_used
      FROM script_keys WHERE script_id=${scriptId} AND user_id=${user.id} ORDER BY created_at DESC
    `;
  ...
  if (req.method === 'POST') {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const [k] = await sql`
        INSERT INTO script_keys (script_id, user_id, key_value, hwid_locked, note, duration, expires_at)
        VALUES (${scriptId}, ${user.id}, ${key_value}, ${hwid_locked}, ${note}, ${duration}, ${expires_at})
        RETURNING id, key_value, hwid_locked, note, duration, expires_at, created_at
      `;
      created.push(k);
    }
