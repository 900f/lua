...
async function logExec(sql, script, robloxName, ip, keyUsed, hwid, success, failReason) {
  try {
    await sql`INSERT INTO executions (script_id,user_id,roblox_name,ip_address,key_used,hwid,success,fail_reason) VALUES (${script.id},${script.user_id},${robloxName||'Unknown'},${ip},${keyUsed},${hwid||null},${success},${failReason||null})`;
  } catch(_) {}
}
...
export default async function handler(req, res) {
  ...
  if (banned) { await logExec(sql,script,robloxName,ip,keyValue,hwid,false,'IP banned'); return res.status(403).json({ s:null, e:'Access denied.' }); }

  let keyUsed = null;
  if (script.key_protected || script.use_key_system) {
    if (!keyValue) { await logExec(sql,script,robloxName,ip,null,hwid,false,'No key'); return res.status(403).json({ s:null, e:'No key provided.' }); }
    ...
    if (!sk || !sk.active) { await logExec(sql,script,robloxName,ip,keyValue,hwid,false,'Invalid key'); return res.status(403).json({ s:null, e:'Invalid or disabled key.' }); }
    ...
    if (sk.hwid_locked) {
      ...
      else if (sk.hwid!==hwid) { await logExec(sql,script,robloxName,ip,keyValue,hwid,false,'HWID mismatch'); return res.status(403).json({ s:null, e:'HWID mismatch.' }); }
