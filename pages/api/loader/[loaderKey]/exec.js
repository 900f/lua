...
export default async function handler(req, res) {
  ...
  if (script.key_protected || script.use_key_system) {
    ...
    const [sk] = await sql`
      ...
      FROM script_keys WHERE script_id=${script.id} AND key_value=${keyValue} LIMIT 1
    `;
    if (!sk || !sk.active) {
  ...
  // Security layers: XOR-obfuscated loader (endpoint URL hidden),
  // HTTPS transport, server-side key/HWID/IP validation.
