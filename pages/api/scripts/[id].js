...
export default async function handler(req, res) {
  ...
  const rl = rateLimit('scrud:'+user.id, 20, 10000);
  ...
  if (req.method === 'PUT') {
    ...
    const [u] = await sql`UPDATE scripts SET name=${name},description=${description},script_content=${script_content},key_protected=${key_protected},use_key_system=${use_key_system},active=${active},updated_at=NOW() WHERE id=${id} AND user_id=${user.id} RETURNING id,name,description,loader_key,key_protected,use_key_system,active,updated_at`;
    return res.json({ script: u });
  }
